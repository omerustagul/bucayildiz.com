 ...
 */
export function Table({ columns = [], rows = [], onRowClick, getRowKey, empty = 'Kayıt bulunamadı.', dense = false, style = {}, ...rest }) {
  const [hover, setHover] = React.useState(-1);
  const pad = dense ? '10px 14px' : '14px 16px';
  return (
    <div style={{ border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', overflowX: 'auto', background: 'var(--surface-card)', ...style }} {...rest}>
      <table style={{ width: '100%', minWidth: 540, borderCollapse: 'collapse', fontFamily: 'var(--font-body)' }}>
          ...
          <tr style={{ background: 'var(--ink-50)', borderBottom: '1px solid var(--border-subtle)' }}>
            {columns.map((c) => (
              <th key={c.key} style={{
                ...
                letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--ink-500)', whiteSpace: 'nowrap',
              }}>{c.label}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.length === 0 && (
            <tr><td colSpan={columns.length} style={{ padding: '40px 16px', textAlign: 'center', color: 'var(--ink-400)', fontSize: 14 }}>{empty}</td></tr>
          )}
          {rows.map((row, i) => (
            <tr key={getRowKey ? getRowKey(row, i) : i}
              ...
              {columns.map((c) => (
                <td key={c.key} style={{
                  ...
                  fontSize: 14, color: 'var(--ink-700)', verticalAlign: 'middle',
