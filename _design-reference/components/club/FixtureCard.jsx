...
function TeamCrest({ crest, name }) {
  if (crest) return <img src={crest} alt={name} style={{ width: 52, height: 52, objectFit: 'contain' }} />;
  const initials = (name || '?').split(' ').map((w) => w[0]).slice(0, 2).join('');
  return (
    <div style={{
      width: 52, height: 52, borderRadius: '50%', display: 'grid', placeItems: 'center',
      background: 'var(--navy-50)', border: '1.5px solid var(--ink-200)',
...
export function FixtureCard({
  competition = 'Lig Maçı', date, venue, status = 'upcoming',
  home = {}, away = {}, style = {}, ...rest
}) {
  const finished = status === 'finished';
  const live = status === 'live';
  const center = live
    ? <span style={{ fontFamily: 'var(--font-stat)', fontWeight: 700, fontSize: 34, color: 'var(--navy-900)', fontVariantNumeric: 'tabular-nums' }}>{home.score ?? 0}–{away.score ?? 0}</span>
    : finished
      ? <span style={{ fontFamily: 'var(--font-stat)', fontWeight: 700, fontSize: 34, color: 'var(--navy-900)', fontVariantNumeric: 'tabular-nums' }}>{home.score ?? 0}–{away.score ?? 0}</span>
      : <span style={{ fontFamily: 'var(--font-stat)', fontWeight: 700, fontSize: 26, color: 'var(--navy-600)', fontVariantNumeric: 'tabular-nums' }}>{home.time || 'VS'}</span>;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: '11px 16px', background: 'var(--navy-800)', color: '#fff',
      }}>
        ...
        {live
          ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#fff' }}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--red-600)', animation: 'byPulse 1.4s var(--ease-in-out) infinite' }} />Canlı
            </span>
